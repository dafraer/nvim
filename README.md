...
- Go toolchain (`go`, `gofmt`, `gopls`); `golangci-lint` optional but recommended
...
4. Install external tools in your PATH: `gofmt` (comes with Go) and `golangci-lint`.
...
- Formatting: gofmt runs on save for Go buffers; there is also an LSP `BufWritePre` formatter for `*.go`.
...
lua/mycfg/*.lua            -- personal tweaks: clipboard, tabs, netrw map, auto-pairs, gofmt on save, diagnostics keys, greeting, etc.
lsp/gopls.lua              -- alternative gopls config using async root detection
```

## Go workflow tips
- Save any Go buffer to run gofmt; if it is not on PATH you will see a warning.
